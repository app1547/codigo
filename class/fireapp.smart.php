        ...
        if($info['estado']){
            ...
            if($result['count'] == 0){
                ...
                if($lat != "" && $lng != ""){
                    ...
                    $s = $this->con->sql("UPDATE actos_user_camino SET lat='".$lat."', lng='".$lng."', lat2='".$lat."', lng_actual='".$lng."', modo='".$modo."', fecha=now(), posicion='1', distancia='".$google['distvalue']."', tiempo='".$google['timevalue']."' WHERE id_act='".$id_act."' AND id_user='".$id_user."'");
                    echo "1<br><br>";
                    print_r($s);
                }
                ...
                if($lat != "" && $lng != ""){
                    if($result['resultado'][0]['posicion'] == 0){
                        $act = $this->con->sql("SELECT lat, lng FROM actos WHERE id_act='".$id_act."'");
                        $google = $this->getgoogledist($act['resultado'][0]['lat'], $lat, $act['resultado'][0]['lng'], $lng, $modo);
                        $this->con->sql("UPDATE actos_user_camino SET lat='".$lat."', lng='".$lng."', lat2='".$lat."', lng_actual='".$lng."', modo='".$modo."', fecha=now(), posicion='1', distancia='".$google['distvalue']."', tiempo='".$google['timevalue']."' WHERE id_act='".$id_act."' AND id_user='".$id_user."'");
                        echo "2";
                    }else{
                        $this->con->sql("UPDATE actos_user_camino SET lat2='".$lat."' AND lng_actual='".$lng."' WHERE id_act='".$id_act."' AND id_user='".$id_user."'");
                        echo "3";
                    }
                }
